...
import { History, Clock, CheckCircle, XCircle, FileText, Loader2, Star } from 'lucide-react';
import { getUserPrintOrders, getUserSubscriptions } from '@/lib/data';
import { useAuth } from '@/hooks/use-auth';
import type { PrintOrder, Subscription } from '@/lib/types';
import { Badge } from '@/components/ui/badge';
import { format } from 'date-fns';
import { cn } from '@/lib/utils';

const PrintOrderCard = ({ order }: { order: PrintOrder }) => {
    const statusConfig = {
        pending: {
            variant: "secondary",
            icon: Clock,
...
export default function PurchaseHistoryPage() {
    ...
    useEffect(() => {
        ...
        ]).then(([userPrintOrders, userSubscriptions]) => {
            setPrintOrders(userPrintOrders);
    ...
    <div className="container mx-auto px-4 py-12">
      <header className="text-center mb-12">
        ...
        <p className="text-muted-foreground text-lg max-w-2xl mx-auto">
          View your print orders and subscription records.
        </p>
      </header>
      <main className="max-w-4xl mx-auto">
        {displayLoading ? (
            ...
            <Card>
             ...
             <Tabs defaultValue="print-orders">
                <TabsList className="grid w-full grid-cols-2">
                    <TabsTrigger value="print-orders">
                        <FileText className="mr-2 h-4 w-4" /> Print Orders
                    </TabsTrigger>
                    <TabsTrigger value="subscriptions">
                        <Star className="mr-2 h-4 w-4" /> Subscriptions
                    </TabsTrigger>
                </TabsList>
                <TabsContent value="print-orders" className="mt-6">
                    {printOrders.length > 0 ? (
                        <div className="space-y-6">
                            {printOrders.map(order => <PrintOrderCard key={order.id} order={order} />)}
                        </div>
                    ) : (
